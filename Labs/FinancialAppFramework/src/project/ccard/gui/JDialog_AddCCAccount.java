...
import project.ccard.CreditFactory;
...
public class JDialog_AddCCAccount extends javax.swing.JDialog {
	 ...
	 * 
	 */
	public JDialog_AddCCAccount(CardFrm thisframe, boolean isNew,
			String _customerName) {
		super(thisframe);

		// Initialize credit object.
		creditFactory = CreditFactory.INSTANCE;

		// Set the parent frame and set the control flags/
		parentframe = thisframe;
		this.isNew = isNew;
		this.customerName = _customerName;

		// Set customer info based on the selected operations (New Account or Add Account)
		if(this.isNew==false && this.customerName!=null){
			setCustomerInfo(_customerName);
		}
		
		// Set the default account type radio button.
		this.JRadioButton_Gold.setSelected(true);
	...
	void JButtonOK_actionPerformed(java.awt.event.ActionEvent event) {
		if (isNew) {
			...
			// Get Account from UI
			account = creditFactory.getInstance(parentframe.accountType,
					JTextField_CCNR.getText(), new Date());
			// Get Address from UI
			address = creditFactory.getAddressInstace(JTextField_STR.getText(),
					JTextField_CT.getText(), JTextField_ST.getText(),
					JTextField_ZIP.getText());
			// Call Save method.
			creditApp.addPersonalCreditAccount(JTextField_NAME.getText(), account,
					address);
			parentframe.LoadListData(creditApp.getCustomers());
			dispose();
		}else {
			...
			// Check the account is existed.
			if(creditApp.isAccountExist(customerName, JTextField_CCNR.getText())){
				JOptionPane.showMessageDialog(this, "Account Number " + JTextField_CCNR.getText() + " Already exist \r\n Please Enter Different Number.", 
						"Warning: Account # Exist", JOptionPane.WARNING_MESSAGE);
			}else {
				account = creditFactory.getInstance(parentframe.accountType,
					JTextField_CCNR.getText(), new Date());
				creditApp.addAccount(account, customerName);
				// Load customer data.
				parentframe.LoadListData(creditApp.getCustomers());
	...
	void ChangeAddressControllState(boolean value) {

		JTextField_NAME.setEditable(value);
		JTextField_STR.setEnabled(value);
		JTextField_CT.setEnabled(value);
		JTextField_ST.setEnabled(value);
		JTextField_ZIP.setEnabled(value);
		JTextField_Email.setEnabled(value);
	}

	void setCustomerInfo(String customerName){
		customer = creditApp.getCustomer(customerName);
		address = customer.getAddresses().get(0);
		JTextField_NAME.setText(customer.getName());
		JTextField_STR.setText(address.getStreet());
		JTextField_CT.setText(address.getCity());
		JTextField_ST.setText(address.getState());
		JTextField_ZIP.setText(address.getZip());
		
		ChangeAddressControllState(false);
