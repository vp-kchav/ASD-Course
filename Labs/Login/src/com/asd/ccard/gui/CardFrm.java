...
public class CardFrm extends javax.swing.JFrame {
	...
	static public void main(String args[]) {
		try {
			...
			AccountView creditCardView = new CreditCardView(accountService);
			CardFrm creditForm = new CardFrm();
			creditForm.setAccountView(creditCardView);
	...
	private void setAccountView(AccountView creditCardView) {
		this.accountView = creditCardView;
	...
	void JButtonGenerateBill_actionPerformed(java.awt.event.ActionEvent event) {
		String report = this.accountView.generateReport();		
	...
	private void createAccount(String type) {
		...
		this.accountView.createAccount(customer, accountType, ccnumber, 0.0);
	}

	private Account withdraw() {
		double amount = Double.valueOf(amountDeposit);
		this.accountView.withdraw(ccnumber, amount);
		Account account = this.accountView.getAccount(ccnumber);
		return account;
	}

	private Account deposit() {
		double amount = Double.valueOf(amountDeposit);
		this.accountView.deposit(ccnumber, amount);
		Account account = this.accountView.getAccount(ccnumber);
		return account;
	}

	public void addInterest() {
		this.accountView.addInterest();
	}
